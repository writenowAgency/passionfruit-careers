...
// Remove experience
router.delete('/experience/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user.id;
    ...
    await profileService.removeExperience(userId, experienceId);
    return res.json({ message: 'Experience removed successfully' });
  } catch (error) {
...
// Remove education
router.delete('/education/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user.id;
    ...
    await profileService.removeEducation(userId, educationId);
    return res.json({ message: 'Education removed successfully' });
  } catch (error) {
    console.error('Remove education error:', error);
    return res.status(500).json({
      message: error instanceof Error ? error.message : 'Failed to remove education',
    });
  }
});
