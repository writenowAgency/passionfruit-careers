...
import { Text, TextInput, Button, Chip, Card, IconButton, Snackbar } from 'react-native-paper';
...
const SkillsForm: React.FC<SkillsFormProps> = ({ profile }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setSnackbarMessage(error instanceof Error ? error.message : 'Failed to add skill');
      setSnackbarVisible(true);
    }
  });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await dispatch(removeSkillAsync(skillToDelete.id)).unwrap();
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <ConfirmModal
        ...
        title="Remove Skill"
        message={`Are you sure you want to remove "${skillToDelete?.name}" from your skills?`}
