...
import { useGetJobsQuery, useGetCategoriesQuery } from '@/store/api/apiSlice';
...
const HomeScreenModern: React.FC = () => {
  ...
  const { data, isFetching, refetch } = useGetJobsQuery(
    selectedCategory ? { categoryId: selectedCategory.toString() } : undefined
  );
  ...
  useEffect(() => {
    CacheService.remove(CACHE_KEY).catch(console.error);
  }, []);
  ...
  const WelcomeHeader = () => {
    const rawUserName = profile.data?.user?.fullName || profile.data?.user?.firstName || 'Job Seeker';
    ...
    // Remove trailing " Name" (common default if last name wasn't provided)
    if (userName.toLowerCase().endsWith(' name')) {
      userName = userName.substring(0, userName.length - 5).trim();
    }
            ...
            >
              <Pressable
                style={[
                  ...
                  selectedCategory === null && styles.collectionCardActive,
                ]}
                onPress={() => setSelectedCategory(null)}
              >
                <View style={[styles.collectionIcon, selectedCategory === null && styles.collectionIconActive]}>
                  <Ionicons name="grid" size={24} color={selectedCategory === null ? colors.primary : colors.textSecondary} />
                </View>
                <Text style={[styles.collectionName, selectedCategory === null && styles.collectionNameActive]}>
                  All Jobs
                </Text>
              </Pressable>
              ...
              {categories?.map((cat) => (
                <Pressable
                  ...
                  style={[
                    styles.collectionCard,
                    selectedCategory === cat.id && styles.collectionCardActive,
                  ]}
                  onPress={() => setSelectedCategory(cat.id === selectedCategory ? null : cat.id)}
                >
                  <View style={[styles.collectionIcon, selectedCategory === cat.id && styles.collectionIconActive]}>
                    <Ionicons name={getCategoryIcon(cat.name) as any} size={24} color={selectedCategory === cat.id ? colors.primary : colors.textSecondary} />
                  </View>
                  <Text style={[styles.collectionName, selectedCategory === cat.id && styles.collectionNameActive]}>
                    {cat.name}
                  </Text>
                  <Text style={styles.collectionCount}>{cat.jobCount} jobs</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
